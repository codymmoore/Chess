...
namespace move
{
	...
	std::vector<Move> getValidMoves(const ChessState& chessState, const Color player)
	{
		...
		for (const PieceNode& piece : pieces)
		{
			...
			case ROOK:
				...
				break;
			case QUEEN:
				getValidMovesLinear(player, piece, chessState, queenDirections, moves);
				break;
			case KING:
		...
		while (move != result.end())
		{
			...
			makeMove(player, *pieceCopy, move->destination, gameCopy);
	...
	void makeMove(const Color player, const PieceNode& piece, const Position& destination, ChessState& chessState, const PieceType promotion)
	{
		...
		if (piece.m_pieceType == PAWN)
		{
			...
			if ((destination.x - piece.m_position.x) != 0 && !chessState.m_board.posIsOccupied(destination))
			{
				Position backward;
			...
			if (destination.y == endOfBoard)
			{
				...
				// Update board
				chessState.m_board.clearPos(piece.m_position, player, PAWN);
				chessState.m_board.addPiece(piece.m_position, player, promotion);
			}
		}
		...
		/* ----- Update move history ----- */
		chessState.m_moveHistory.push_back(MoveHistoryNode(player, piece, destination));
		...
		/* ----- Move Piece ----- */
		// Update board
		chessState.m_board.clearPos(piece.m_position, player, pieceRef.m_pieceType);
