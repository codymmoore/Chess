...
namespace getValidMovesTest
{
	...
	namespace check
	{
		TEST_F(GetValidMovesTest, removeMovesThatResultInCheck_pawm_white)
		{
			...
			chessState = std::make_unique<ChessState>("4k3/8/8/8/8/8/6p1/4K3 w - - 0 1");
		...
		TEST_F(GetValidMovesTest, removeMovesThatResultInCheck_pawn_black)
		{
			...
			chessState = std::make_unique<ChessState>("4k3/6P1/8/8/8/8/8/4K3 b - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
	...
	namespace pawn
	{
		TEST_F(GetValidMovesTest, pawnMoveForward_white)
		{
			...
			chessState = std::make_unique<ChessState>("4k3/8/8/8/8/8/3P4/RNBQKBNR w - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
		...
		TEST_F(GetValidMovesTest, pawnMoveForward_black)
		{
			...
			chessState = std::make_unique<ChessState>("rnbqkbnr/3p4/8/8/8/8/8/4K3 b - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
			const std::vector<Move> pawnMoves = getPieceMoves(validMoves, SOURCE);
			const std::vector<Move> expectedMoves = {
				Move(SOURCE, SOURCE + forward(COLOR)),
		...
		TEST_F(GetValidMovesTest, pawnMoveForward_outOfBoundsAbove)
		{
			...
			chessState = std::make_unique<ChessState>("3Pk3/8/8/8/8/8/8/4K3 w - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
		...
		TEST_F(GetValidMovesTest, pawnMoveForward_outOfBoundsBelow)
		{
			...
			chessState = std::make_unique<ChessState>("4k3/8/8/8/8/8/8/4Kp2 b - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
			const Move invalidMove(SOURCE, SOURCE + forward(COLOR));
			EXPECT_THAT(validMoves, Not(Contains(invalidMove)));
		}
		...
		TEST_F(GetValidMovesTest, pawnCapture)
		{
			const Color COLOR = Color::WHITE;
			const Position SOURCE = Position(3, 6);
			chessState = std::make_unique<ChessState>("4k3/8/8/8/8/2p1p3/3P4/RNBQKBNR w - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
			EXPECT_TRUE(chessState->getBoard().posIsOccupied(SOURCE + forward(COLOR) + RIGHT, ~COLOR));
			EXPECT_THAT(validMoves, Contains(Move(SOURCE, SOURCE + forward(COLOR) + RIGHT)));
			EXPECT_TRUE(chessState->getBoard().posIsOccupied(SOURCE + forward(COLOR) + LEFT, ~COLOR));
		...
		TEST_F(GetValidMovesTest, pawnEnPassant_right)
		{
			...
			chessState = std::make_unique<ChessState>("4k3/8/8/3Pp3/8/8/8/RNBQKBNR w - e6 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
			EXPECT_THAT(validMoves, Contains(Move(SOURCE, SOURCE + forward(COLOR) + RIGHT)));
		}
		...
		TEST_F(GetValidMovesTest, pawnEnPassant_left)
		{
			const Color COLOR = Color::WHITE;
			const Position SOURCE = Position(3, 3);
			chessState = std::make_unique<ChessState>("4k3/8/8/2pP4/8/8/8/RNBQKBNR w - c6 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
			EXPECT_THAT(validMoves, Contains(Move(SOURCE, SOURCE + forward(COLOR) + LEFT)));
		}
		...
		TEST_F(GetValidMovesTest, pawnEnPassant_noMoveHistory)
		{
			const Color COLOR = Color::WHITE;
			const Position SOURCE = Position(3, 3);
			chessState = std::make_unique<ChessState>("4k3/8/8/3Pp3/8/8/8/RNBQKBNR w - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
		...
		TEST_F(GetValidMovesTest, pawn_blocked1)
		{
			const Color COLOR = Color::WHITE;
			const Position SOURCE = Position(3, 6);
			chessState = std::make_unique<ChessState>("4k3/8/8/8/8/3p4/3P4/RNBQKBNR w - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
			EXPECT_EQ(
		...
		TEST_F(GetValidMovesTest, pawn_blocked2)
		{
			const Color COLOR = Color::WHITE;
			const Position SOURCE = Position(3, 6);
			chessState = std::make_unique<ChessState>("4k3/8/8/8/3p4/8/3P4/RNBQKBNR w - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
	...
	namespace knight
	{
		TEST_F(GetValidMovesTest, knight_moves)
		{
			const Color COLOR = Color::WHITE;
			const Position SOURCE = Position(3, 3);
			chessState = std::make_unique<ChessState>("4k3/8/8/3N4/8/8/8/4K3 w - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
		...
		TEST_F(GetValidMovesTest, knight_outOfBoundsUpAndRight)
		{
			...
			chessState = std::make_unique<ChessState>("4k2N/8/8/8/8/8/8/4K3 w - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
			const std::vector<Move> expectedMoves = {
				Move(SOURCE, SOURCE + DOWN * 2 + LEFT),
				Move(SOURCE, SOURCE + LEFT * 2 + DOWN)
		...
		TEST_F(GetValidMovesTest, knight_outBoundsLeftAndDown)
		{
			...
			chessState = std::make_unique<ChessState>("4k3/8/8/8/8/8/8/N3K3 w - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
			const std::vector<Move> expectedMoves = {
				Move(SOURCE, SOURCE + UP * 2 + RIGHT),
				Move(SOURCE, SOURCE + RIGHT * 2 + UP)
		...
		TEST_F(GetValidMovesTest, knight_capture)
		{
			...
			chessState = std::make_unique<ChessState>("4k3/8/5p2/3N4/8/8/8/4K3 w - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
	...
	namespace bishop
	{
		TEST_F(GetValidMovesTest, bishop)
		{
			const Color COLOR = Color::WHITE;
			const Position SOURCE = Position(3, 3);
			chessState = std::make_unique<ChessState>("4k3/8/8/3B4/8/8/8/4K3 w - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
		...
		TEST_F(GetValidMovesTest, bishop_blocked)
		{
			const Color COLOR = Color::WHITE;
			const Position SOURCE = Position(3, 3);
			chessState = std::make_unique<ChessState>("4k3/1P7/4P3/3B4/2P5/5P2/8/4K3 w - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
	...
	namespace rook
	{
		TEST_F(GetValidMovesTest, rook)
		{
			const Color COLOR = Color::WHITE;
			const Position SOURCE = Position(3, 3);
			chessState = std::make_unique<ChessState>("4k3/8/8/3R4/8/8/8/4K3 w - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
		...
		TEST_F(GetValidMovesTest, rook_blocked)
		{
			const Color COLOR = Color::WHITE;
			const Position SOURCE = Position(3, 3);
			chessState = std::make_unique<ChessState>("4k3/8/3P4/2PR1P1/8/3P4/8/4K3 w - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
			const std::vector<Move> invalidMoves = {
				Move(SOURCE, Position(3, 2)),
				Move(SOURCE, Position(2, 3)),
	...
	namespace queen
	{
		TEST_F(GetValidMovesTest, queen)
		{
			const Color COLOR = Color::WHITE;
			const Position SOURCE = Position(3, 3);
			chessState = std::make_unique<ChessState>("4k3/8/8/3Q4/8/8/8/4K3 w - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
		...
		TEST_F(GetValidMovesTest, queen_blocked)
		{
			const Color COLOR = Color::WHITE;
			const Position SOURCE = Position(3, 3);
			chessState = std::make_unique<ChessState>("4k3/1P7/3PP3/2PQ1P1/2P5/3P1P2/8/4K3 w - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
			const std::vector<Move> invalidMoves = {
				Move(SOURCE, Position(4, 2)),
				Move(SOURCE, Position(1, 1)),
	...
	namespace king
	{
		TEST_F(GetValidMovesTest, king_all)
		{
			const Color COLOR = Color::WHITE;
			const Position SOURCE = Position(3, 3);
			chessState = std::make_unique<ChessState>("4k3/8/8/3K4/8/8/8/8 w - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
			const std::vector<Move> expectedMoves = {
				Move(SOURCE, Position(2, 2)),
				Move(SOURCE, Position(3, 2)),
		...
		TEST_F(GetValidMovesTest, king_capture)
		{
			...
			chessState = std::make_unique<ChessState>("4k3/8/8/3Kp3/8/8/8/8 w - - 0 1");
			const std::vector<Move> validMoves = getValidMoves(*chessState, COLOR);
			EXPECT_TRUE(chessState->getBoard().posIsOccupied(DESTINATION, ~COLOR));
			EXPECT_THAT(validMoves, Contains(Move(SOURCE, DESTINATION)));
		}
